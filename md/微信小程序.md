...
#### 2. 微信小程序 promise 化
```javascript{.line-numbers}
1. npm i --save miniprogram-api-promise  // 下载promise 化的库
2. import { promisifyAll } from ' miniprogram-api-promise'; // 通过 ES6 方法，引入库；
3. const wxp = wx.p = {}; // 声明一个承载 wx api promise 化的空对象；
4. promisifyAll(wx, wxp); //  传入 wx 顶级对象和空对象

使用：
    async getInfo(){
        const {data:res} = wx.p.request({
            methods:'GET',
...
使用 mobx-miniprogram-bindings:
  ...
  2. 绑定到组件
    //组件.js 文件
    // 引入 mobx-miniprogram-bindings 库；
    import {storeBindingsBehaviors} from 'mobx-miniprogram-bindings';
    // 引入 store 文件；
    import {store} from "../store/store";
    Component({
        behaviors:[storeBindingsBehaviors],
        storeBindings:{
            store, // 数据源
            fields:{
