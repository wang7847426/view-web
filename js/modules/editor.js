	...
	Editor.prototype = {
		...
		bindEvent:function(){
			...
			if(self.editorObj[0].addEventListener){
				self.editorObj[0].addEventListener("paste", function(){self.pasteHandler.call(self)}, false);
			}else{
				self.editorObj[0].attachEvent("onpaste", function(){self.pasteHandler.call(self)});
			}
			//焦点定位
			this.saveRange();
			self.editorObj.bind({
			    keyup:function(){self.saveRange.call(self)},
			    mouseup:function(){self.saveRange.call(self)},
			    change:function(){self.saveRange.call(self)}
			});
			//绑定插入表情
			this.clickFace();
			//绑定插入链接url
